...
angular.module("time-input", []).directive("timeInput", [ function TimeInput() {
  return {
    ...
    link: function(scope, element, attrs, ngModel) {
      var $inputs = element.find("input"), $hourInput = $inputs.eq(0), $minuteInput = $inputs.eq(1), hours = 0, minutes = 0, afterBlur;
      ...
      function shouldUpdateOnChange() {
        return !(ngModel.$options && ngModel.$options.updateOn && ngModel.$options.updateOn.indexOf("default") === -1);
      }
      ...
      $hourInput.on("change", function onHourChange() {
      ...
      $minuteInput.on("change", function onMinuteChange() {
        if (this.value && !isNaN(this.value)) {
